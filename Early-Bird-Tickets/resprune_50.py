...
if args.dataset == 'imagenet':
    model = models.__dict__['resnet50_official'](pretrained=False)
...
if args.model:
    if os.path.isfile(args.model):
        ...
        model.load_state_dict(fix_robustness_ckpt(checkpoint['state_dict']))
...
if args.dataset == 'imagenet':
    ...
    print(new_cfg)
    newmodel = models.__dict__[args.arch](pretrained=False, cfg=new_cfg)
...
#             # print(len(start_mask), len(end_mask))
#             bn_count += 1
...
#                     m2 = new_modules[useful_i[layer_id+6]]
#                     assert isinstance(m2, channel_selection)
#                 else:
#                     if layer_id + 4 < len(useful_i):
#                         m2 = new_modules[useful_i[layer_id+4]]
...
#                     end_mask = cfg_mask[layer_id_in_cfg]
#         # elif layer_id in channel_select[:-1]:
#         #     # We need to set the channel selection layer.
#         #     _mask = original_cfg_mask[channel_select_bn[channel_select.index(layer_id)] - 1]
#         #     idx1 = np.squeeze(np.argwhere(np.asarray(_mask.cpu().numpy())))
#         #     m2 = new_modules[useful_i[layer_id]]
#         #     assert isinstance(m2, channel_selection)
#         #     if isinstance(m2, channel_selection):
#         #         m2.indexes = np.zeros_like(m2.indexes)
#         #         m2.indexes[idx1.tolist()] = 1.0
...
torch.save({'cfg': new_cfg, 'state_dict': newmodel.state_dict()}, os.path.join(args.save, 'pruned.pth.tar'))
